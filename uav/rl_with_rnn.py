# 作用: 提供 PointerNet/RNN 基线策略网络; 依赖: Environment/State.py、modules.py; 被依赖: solver.py、Model_Loader.py
# RNN模型实现了一个基于RNN（循环神经网络）和注意力机制的旅行商问题（TSP）解算器模型，具体来说是通过LSTM（长短期记忆网络）和注意力机制来进行任务序列的选择和路径规划
...
class RNNTSP(nn.Module):
    ...
    def apply_mask_to_logits(self, logits, mask, idxs):  # 确保在选择动作时，某些特定位置不会被模型考虑，这对于避免重复选择或满足某些约束条件非常有用
