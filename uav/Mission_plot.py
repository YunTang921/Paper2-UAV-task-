# 作用: 采样单个任务场景并可视化覆盖/巡检/取放任务分布; 依赖: Environment/Mission.py
# Purpose: sample a single mission scene and visualize task distribution; Dependencies: Environment/Mission.py
...
def task_plot(tasks, args, save_dir=None):
  fig, ax = plt.subplots()
  ...
  # optional extra save without changing original behavior
  if save_dir is not None:
    Path(save_dir).mkdir(parents=True, exist_ok=True)
    ts = int(time.time())
    out_path = Path(save_dir) / f"mission_plot_{ts}.png"
    fig.savefig(out_path, bbox_inches="tight", dpi=200)
    print(f"Saved mission plot to {out_path}")
...
def test(args):
    ...
    for  (batch_idx, task_list_batch) in train_loader:
        # 显示并额外保存到 new results 目录
        task_plot(task_list_batch[0].cpu().numpy(), args, save_dir="new results")
