# 作用: 生成覆盖/巡检/取放任务数据集供训练评估; 依赖: 无其它项目文件;
# 被依赖: Mission_plot.py、baseline_analyze.py、test_or_tools.py、train_mission.py 等任务加载脚本
...
#MissionDataset_2 中生成拾取与放置任务时，偏移量的范围从 np.random.uniform(0.02, 0.09, 2) 改为 np.random.uniform(0.02, 0.1, 2)，稍微增加了偏移量的最大值
...
#检查生成的任务示例数据是否符合预期，并验证数据加载过程是否正常工作。
def test(args):
...
if __name__ == "__main__":#只有在直接运行该脚本时，下面的代码才会被执行。如果这个脚本被导入到另一个脚本中运行，下面的代码将不会被执行
    import argparse
