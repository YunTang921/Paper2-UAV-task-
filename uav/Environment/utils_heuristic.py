# 作用: 提供异构任务成本计算及 OR-Tools/GREEDY 基线; 依赖: 无其它项目文件; 被依赖: baseline_analyze.py、test_or_tools.py
# 多种算法用于解决车辆路径问题（VRP），其中包括了一些计算路径成本的函数，以及使用 Google OR-Tools 和贪心算法来求解问题
import numpy as np
import copy
import time
...
def cal_cost(source_task, target_task):
    ...
    if target_task[-1] == 1 or target_task[
        -3] == 1:  # Area, center position + area_info / Delivery, pick position + area_info
...
def ORTOOLS_TYPE2(pointset, dist_constraint=None, vehicle_num=None, scale=1000.0, time_verbose=False):
    ...
    def distance_callback(from_index, to_index):
        # OR-Tools callback expects Python ints and int return
...
# q = lkh.solve(solver_path, problem=problem_instance, runs=100, max_candidates="6 symmetric", move_type=3,
# patching_c=3, patching_a=2, trace_level=0) q = np.array(q[0])-1
