# 作用: 定义 HetNet 及其 Partial/t-SNE 变体的策略网络;
# 依赖: modules.py、Environment/State.py; 被依赖: solver.py、Model_Loader.py、train_mission.py
import torch
...
class att_layer(nn.Module):
    def __init__(self, embed_dim, n_heads, feed_forward_hidden=512, bn=False):
        ...
        self.embed = nn.Sequential(nn.Linear(embed_dim, feed_forward_hidden), nn.ReLU(),
...
class HetNet_tSNE(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return torch.stack(prev_chosen_logprobs, 1), torch.stack(prev_chosen_indices, 1), embedded.reshape(-1,
                                                                                                           self.embedding_size)
