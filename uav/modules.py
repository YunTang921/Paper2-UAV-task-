# 作用: 提供图嵌入和注意力/指针模块供模型复用; 依赖: 无其它项目文件; 被依赖: rl_mission_planning.py、rl_with_rnn.py
...
class Glimpse(nn.Module):
    ...
    def forward(self, query, target, mask=None):
        ...
        k = self.W_k(target).reshape(batch_size, seq_len, self.n_head, self.single_dim).permute(0, 2, 1,
                                                                                                3).contiguous()  # batch x head x seq_len x head_dim
        v = self.W_v(target).reshape(batch_size, seq_len, self.n_head, self.single_dim).permute(0, 2, 1,
                                                                                                3).contiguous()  # batch x head x seq_len x head_dim
