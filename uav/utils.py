# 作用: 提供异构任务成本、奖励与外部求解辅助函数; 依赖: 无其它项目文件; 被依赖: Environment/State.py、solver.py、baseline_analyze.py、test_or_tools.py 等
import tsplib95
import lkh
...
def Dae_Bup_Gwan(mission_instance, solution):
    ...
    def route_cost(route):
        ...
        for i in range(len(r)):
            cost += ret_matrix[r[i], r[(i + 1) % len(r)]]
        return cost

    # Accept single route (list/array) or list of routes
    if isinstance(solution, (list, tuple)) and solution and isinstance(solution[0], (list, tuple, np.ndarray)):
        total = sum(route_cost(r) for r in solution)
    else:
        total = route_cost(solution)

    return total / CONST, ret_matrix / CONST
...
if __name__ == "__main__":
    ...
    for i, tasksets in tqdm(train_loader):
        ...
        # TEST PART
        state = TA_State.initialize(tasksets.cpu())
